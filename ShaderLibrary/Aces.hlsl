...
{
	x = clamp(x, 0.0, HalfMax);

    // x is clamped to [0, HalfMax], skip the <= 0 check
...
half ACEScc_to_ACES(half x)
{
	...
	else if (x < (log2(HalfMax) + 9.72) / 17.52)
		return pow(2.0, x * 17.52 - 9.72);
	else // (x >= (log2(HalfMax) + 9.72) / 17.52)
		return HalfMax;
}
...
half segmented_spline_c5_fwd(half x)
{
	...
	float xCheck = x;
	if (xCheck <= 0.0)
		xCheck = 0.00006103515; // = pow(2.0, -14.0);
...
half3 RRT(half3 aces)
{
	...
	aces = clamp(aces, 0.0, HalfMax); // avoids saturated negative colors from becoming positive in the matrix
	half3 rgbPre = mul(AP0_2_AP1_MAT, aces);
	rgbPre = clamp(rgbPre, 0, HalfMax);
